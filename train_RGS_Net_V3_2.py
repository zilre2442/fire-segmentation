...
from loss import SpatialFocalTverskyLoss, MaskedL1Loss, FocalTverskyLoss, get_criterion_info
...
#    或者显式设置可见 GPU 设备:
#
#    CUDA_VISIBLE_DEVICES=3,4,5,6,7 torchrun --nproc_per_node=5 train_RGS_Net_V3_2.py --fire-category 
#
#    使用按火点数量划分的子数据集（例如训练 very_few 类别）:
#
#    torchrun --nproc_per_node=4 train_RGS_Net_V3_2.py --fire-category very_few
#
#    或训练 many 类别的样本:
#
#    torchrun --nproc_per_node=4 train_RGS_Net_V3_2.py --fire-category many --epochs 150
#
# 2) 单卡快速调试（需要手动设置环境变量供脚本读取）:
#
#    RANK=0 WORLD_SIZE=1 LOCAL_RANK=0 CUDA_VISIBLE_DEVICES=1 python3 train_RGS_Net_V3_2.py
#
#    使用 few 类别数据集:
#
#    RANK=0 WORLD_SIZE=1 LOCAL_RANK=0 python3 train_RGS_Net_V3_2.py --fire-category few
#
# 3) 其他可选参数:
...
SAVE_DIR = f"output/RGS_Net_V3_2/{ALGORITHM}_{RUN_ID}"
BANDS = (7, 6, 5)
BATCH_SIZE = 64  # 对齐 V3_1 训练脚本的默认批大小
NUM_WORKERS = 4
SHUFFLE_TRAIN = True
EPOCHS = 200
LEARNING_RATE = 3e-4
VAL_INTERVAL = 1
SAVE_INTERVAL = 10
EARLY_STOPPING_PATIENCE = 20  # 对齐 V3_1 的早停耐心
...
# 但通过权重系数实现渐进式训练效果
#  - 阶段1 (epoch 0 ~ MULTI_SCALE_STAGE1_EPOCH-1): 多尺度损失权重=0.2（弱监督）
#  - 阶段2 (epoch MULTI_SCALE_STAGE1_EPOCH ~ MULTI_SCALE_STAGE2_EPOCH-1): 多尺度损失权重=0.6（中等监督）
#  - 阶段3 (epoch >= MULTI_SCALE_STAGE2_EPOCH): 多尺度损失权重=1.0（全监督）
MULTI_SCALE_STAGE1_EPOCH = 20   # 阶段2起始
MULTI_SCALE_STAGE2_EPOCH = 50   # 阶段3起始
# 各阶段的多尺度损失权重
MULTI_SCALE_WEIGHT_STAGE0 = 0.1  # 阶段1：弱监督（对齐 V3_1）
MULTI_SCALE_WEIGHT_STAGE1 = 0.5  # 阶段2：中等监督（对齐 V3_1）
MULTI_SCALE_WEIGHT_STAGE2 = 1.0  # 阶段3：全监督


def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="RGS-Net V3_2 分布式训练脚本")
...
def setup_logging(rank: int, save_dir: str, enable_console: bool = False) -> logging.Logger:
    logger = logging.getLogger(f"rgs_v3_2_train_rank_{rank}")
...
def save_hyperparameters(
    ...
    with open(params_path, "w") as f:
        ...
        f.write("重建分支损失: MaskedL1Loss (使用输入影像灰度并以掩膜屏蔽火点)\n\n")
...
def train(rank: int, world_size: int, args) -> None:
    ...
    if args.save_dir:
        ...
        save_dir = f"output/RGS_Net_V3_2/{algorithm}{category_suffix}_{RUN_ID}"
    ...
    try:
        ...
        # 显式指定 device_ids 为 [local_rank]，确保 DDP 绑定到正确的 GPU
        # 注意：local_rank 是当前进程在节点内的序号（0, 1, ...），对应 CUDA_VISIBLE_DEVICES 中的索引
        ddp_model = DDP(model, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True)

        # 分割分支使用 SpatialFocalTverskyLoss （参数与 V3_1 保持一致，便于比较）
        ...
        # 重建分支使用 MaskedL1Loss（V3_2 为 1 通道重建，使用输入影像灰度对齐）
        recon_criterion = MaskedL1Loss(reduction="mean")
        # 记录损失函数参数（仅rank0，以免重复）
        if rank == 0:
            logger.info("[Segmentation Criterion]\n%s", get_criterion_info(seg_criterion))
            logger.info("[Reconstruction Criterion]\n%s", get_criterion_info(recon_criterion))
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (images, masks) in enumerate(train_iter):
                ...
                # 基础分支损失（与 V3_1 对齐）：seg + rec + fused
                seg_loss = seg_criterion(outputs.seg_logits, masks)
                rec_loss = recon_criterion(outputs.rec_logits, images_gray, masks)
                fused_loss = seg_criterion(outputs.fused_logits, masks)

                # 多尺度损失权重阶段策略（与 V3_1 对齐）
                ...
                if ms_rec:
                    ...
                    for scale_logits in ms_rec:
                        if scale_logits.shape[2:] != images_gray.shape[2:]:
                            scale_logits = F.interpolate(scale_logits, size=images_gray.shape[2:], mode='bilinear', align_corners=False)
                        ms_rec_losses.append(recon_criterion(scale_logits, images_gray, masks))
                    multi_scale_rec_loss = (sum(ms_rec_losses) / len(ms_rec_losses)) * ms_weight

                # 总损失（与 V3_1 形式保持一致，但重建使用灰度目标）
                ...
                # 记录：seg 分支（包含 seg + fused），rec 分支（仅主重建）
                ...
                if rank == 0:
                    train_iter.set_postfix(
                        ...
                        fused=f"{fused_loss.detach().item():.4f}",
                        ms_seg=f"{multi_scale_seg_loss:.4f}",
                        ms_rec=f"{multi_scale_rec_loss:.4f}",
            ...
            logger.info(
                "Rank %d: Epoch %d/%d | Avg Train Loss: %.6f (seg+fused: %.6f, rec: %.6f)",
                rank,
                epoch + 1,
                epochs,
                epoch_loss,
            ...
            if epoch % VAL_INTERVAL == 0:
                    ...
                    for images, masks in val_iter:
                        ...
                        if ms_seg:
                            ...
                            for scale_logits in ms_seg:
                                if scale_logits.shape[2:] != masks.shape[2:]:
                                    scale_logits = F.interpolate(scale_logits, size=masks.shape[2:], mode='bilinear', align_corners=False)
                                tmp.append(seg_criterion(scale_logits, masks))
                            multi_scale_seg_loss = (sum(tmp) / len(tmp)) * ms_weight
                        if ms_rec:
                            tmp = []
                            for scale_logits in ms_rec:
                                if scale_logits.shape[2:] != images_gray.shape[2:]:
                                    scale_logits = F.interpolate(scale_logits, size=images_gray.shape[2:], mode='bilinear', align_corners=False)
                                tmp.append(recon_criterion(scale_logits, images_gray, masks))
                            multi_scale_rec_loss = (sum(tmp) / len(tmp)) * ms_weight
                        ...
                        if rank == 0:
                            val_iter.set_postfix(
                                ...
                                fused=f"{fused_loss.item():.4f}",
                                ms_seg=f"{multi_scale_seg_loss:.4f}",
                                ms_rec=f"{multi_scale_rec_loss:.4f}",
                                rec_w=f"{ddp_model.module.rec_weight.item():.4f}" if hasattr(ddp_model.module, 'rec_weight') else "N/A"
                            )
                ...
                logger.info(
                    "Rank %d: Epoch %d/%d | Avg Val Loss: %.6f (seg+fused: %.6f, rec: %.6f)",
                    rank,
                    epoch + 1,
                    epochs,
                    avg_val_loss,
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        main_logger = logging.getLogger("rgs_v3_2_main")
        main_logger.info(f"Starting distributed RGS-Net V3_2 training with {world_size} GPUs via torchrun")
        ...
        save_dir = args.save_dir if args.save_dir else f"output/RGS_Net_V3_2/{args.algorithm}{category_suffix}_{RUN_ID}"
