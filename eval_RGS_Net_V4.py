...
示例：
CUDA_VISIBLE_DEVICES=1,4,5,6,7 torchrun --nproc_per_node=5 --master_port=65530 eval_RGS_Net_V4.py --dist --data-root data/splits_activefire --algo voting --bands 7 6 5 --output-type fused --batch-size 16 --save-dir output/RGS_Net_V4/voting_202511281514
python eval_RGS_Net_V4.py --model-path output/RGS_Net_V4/Land8Fire_202511261606/weights/model_best.pth --algo voting --bands 7 6 5 --output-type fused --threshold 0.5 --save-dir output/RGS_Net_V4/Land8Fire_202511261606
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate RGS-Net V4 (aligned with V3_1)")
    ...
    p.add_argument("--save-dir", type=str, default="output/RGS_Net_V4/voting_202511252145", help="训练输出目录，用于推断权重路径和保存评估结果")
    p.add_argument("--output-type", type=str, choices=["fused", "rec", "seg"], default="fused")
...
def main():
    ...
    if is_main_process():
        print("\n[阶段 2/4] 加载预训练模型")
    # 权重路径推断：优先使用 --model-path，否则默认 save-dir/weights/model_best.pth
    ...
    model = RGSNetV4(n_channels=len(BANDS)).to(device)
    if is_main_process():
    ...
    # pred_samples: 保存RGB/GT/Seg/Rec/Fused（与V3_1对齐）
    if is_main_process():
        ...
        for i in range(rows):
            ...
            axes[i, 0].imshow(rgb_img); axes[i, 0].set_title("Input (RGB)", fontsize=10); axes[i, 0].axis("off")
