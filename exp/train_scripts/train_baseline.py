...
示例：
CUDA_VISIBLE_DEVICES=2,7 torchrun --nproc_per_node=2 --master_port=65531 exp/train_scripts/train_baseline.py --data-root data/splits_merged_pixels --algo large
CUDA_VISIBLE_DEVICES=3 python exp/train_scripts/train_baseline.py --data-root data/splits_activefire --algo voting --bands 7 6 5
python exp/train_scripts/train_baseline.py --data-root data/splits_merged_pixels --algo small
python exp/train_scripts/train_baseline.py --data-root data/splits_merged_pixels --algo large
"""
...
def main():
    ...
    save_dir = args.save_dir or f"output/baseline/{args.algo}_{time.strftime('%Y%m%d%H%M')}"
    ...
    model = UNet(n_channels=len(args.bands), n_classes=1).to(device)
    if dist_enabled:
    ...
    # criterion = FocalLoss(alpha=0.75, gamma=2.0, reduction="mean")
    ...
    if (not dist_enabled) or rank == 0:
        ...
        with open(hparam_path, "w") as f:
            f.write("===== Baseline UNet Hyperparameters =====\n")
    ...
    if (not dist_enabled) or rank == 0:
        ...
        plt.xlabel("epoch"); plt.ylabel("loss"); plt.title("Baseline UNet Loss")
