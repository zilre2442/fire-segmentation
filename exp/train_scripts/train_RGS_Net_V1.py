...
from loss import MaskedL1Loss, get_criterion_info
...
# 使用示例：CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 exp/train_scripts/train_RGS_Net_V1.py

# 全局超参数配置
DATA_ROOT = "data/full"
ALGORITHM = "voting"  # 可选: 'Kumar-Roy', 'Murphy', 'Schroeder', 'intersection', 'voting'
RUN_ID = datetime.now().strftime("%Y%m%d%H%M")
SAVE_DIR = f"output/RGS_Net_V1/{ALGORITHM}_{RUN_ID}"
BANDS = (7,6,2)  # 使用的波段组合
RECON_LOSS_WEIGHT = 1.0  # 重建损失权重
TAU = 1.0  # 融合温度系数（越大越平滑）
BATCH_SIZE = 64
NUM_WORKERS = 4
SHUFFLE_TRAIN = True
EPOCHS = 200
...
def setup_logging(rank: int, save_dir: str, enable_console: bool = False) -> logging.Logger:
    logger = logging.getLogger(f"rgs_v1_train_rank_{rank}")
...
def train(rank: int, world_size: int) -> None:
    ...
    # 不在终端输出日志，仅写文件；进度显示交给 tqdm（仅 rank0）
    ...
    try:
        ...
        model = RGSNetV1(n_channels=len(BANDS), n_classes=1, n_filters=16, tau=TAU).to(device)
        ...
        for epoch in range(EPOCHS):
            ...
            for batch_idx, (images, masks) in enumerate(train_iter):
                ...
                seg_loss = seg_criterion(outputs.seg_probs, masks)
                rec_loss = recon_criterion(outputs.reconstruction, images, masks)
                loss = seg_loss + RECON_LOSS_WEIGHT * rec_loss
                loss.backward()
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        main_logger = logging.getLogger("rgs_v1_main")
        main_logger.info(f"Starting distributed RGS-Net V1 training with {world_size} GPUs via torchrun")
