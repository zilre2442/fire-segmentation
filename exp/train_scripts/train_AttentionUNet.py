"""训练脚本: Attention UNet (对齐 Dual-Sight Fire 工作流)

- 支持单机/分布式训练 (`--dist` 或 torchrun 环境变量自动检测)
- 记录 `train_log.txt`、`hyperparameters.txt` 以及 `loss_curves.png`

示例：
CUDA_VISIBLE_DEVICES=0,1,2,7 torchrun --nproc_per_node=4 --master_port=65531 exp/train_scripts/train_AttentionUNet.py --data-root data/splits_merged_pixels --algo large
CUDA_VISIBLE_DEVICES=6 python exp/train_scripts/train_AttentionUNet.py --data-root data/splits_merged_pixels --algo small
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train Attention UNet (aligned with Dual-Sight Fire script)")
...
def main():
    ...
    save_dir = args.save_dir or f"output/AttentionUNet/{args.algo}_{time.strftime('%Y%m%d%H%M')}"
    ...
    model = AttentionUNet(n_channels=len(args.bands), n_classes=1).to(device)
    if dist_enabled:
    ...
    if (not dist_enabled) or rank == 0:
        ...
        with open(hparam_path, "w") as f:
            f.write("===== Attention UNet Hyperparameters =====\n")
    ...
    if (not dist_enabled) or rank == 0:
        ...
        plt.xlabel("epoch"); plt.ylabel("loss"); plt.title("Attention UNet Loss")
