"""训练脚本: Dual-Sight Fire StageFusion 变体

特性概述:
- 双解码器结构与 Dual-Sight Fire 相同，但在两个解码器的中间三次 UpBlock 之后执行一次可学习权重的差分融合。
- 主损失使用 fused_logits 进行 SpatialFocalTverskyLoss 监督；辅助损失为 seg_logits 与 rec_logits。

示例命令:
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=65530 exp/train_scripts/train_DualSight_Fire_stagefusion.py --data-root data/splits_activefire --algo voting
CUDA_VISIBLE_DEVICES=7 python exp/train_scripts/train_DualSight_Fire_stagefusion.py --data-root data/splits_activefire --algo voting
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train Dual-Sight Fire StageFusion (single or multi-GPU)")
...
def main():
    ...
    save_dir = args.save_dir or f"output/DualSight_Fire_stagefusion/{args.algo}_{time.strftime('%Y%m%d%H%M')}"
    ...
    model = DualSight_FireStageFusion(
    ...
    if (not dist_enabled) or rank == 0:
        ...
        with open(hparam_path, "w") as f:
            f.write("===== Dual-Sight Fire StageFusion Hyperparameters =====\n")
    ...
    if (not dist_enabled) or rank == 0:
        ...
        plt.xlabel("epoch")
        plt.ylabel("loss")
        plt.title("Dual-Sight Fire StageFusion Loss")
        plt.legend()
        plt.grid(True)
