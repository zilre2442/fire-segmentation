...
from loss import SpatialFocalTverskyLoss, get_criterion_info, FocalTverskyLoss, FocalLoss, TverskyLoss, DiceLoss, SpatialFocalLoss, FocalDiceLoss, SpatialFocalDiceLoss, SpatialFocalBCELoss


def parse_args():
    p = argparse.ArgumentParser(description="Train Dual-Sight Fire (single or multi-GPU)")
...
def main():
    ...
    save_dir = args.save_dir or f"output/DualSight_Fire/{args.algo}_{time.strftime('%Y%m%d%H%M')}"
    ...
    train_ds = LandsatFireDataset(train_csv, bands=tuple(args.bands))  # 训练集
    val_ds = LandsatFireDataset(val_csv, bands=tuple(args.bands))  # 验证集
    ...
    # 模型初始化
    model = DualSight_Fire(n_channels=len(args.bands), base_filters=args.base_filters).to(device)
    if dist_enabled:
    ...
    # )
    # 为数值稳定性，使用 BCEWithLogitsLoss（直接对 logits 计算，避免 sigmoid+BCELoss 的两步误差）
    ...
    if (not dist_enabled) or rank == 0:
        ...
        with open(hparam_path, "w") as f:
            f.write("===== Dual-Sight Fire Hyperparameters =====\n")
            ...
            f.write("loss_weights: fused(0.6) seg(0.2) rec(0.2) [fixed]\n")
    ...
    if (not dist_enabled) or rank == 0:
        ...
        plt.xlabel("epoch"); plt.ylabel("loss"); plt.title("Dual-Sight Fire Loss")
