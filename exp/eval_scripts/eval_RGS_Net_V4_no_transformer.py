"""评估脚本: RGS-Net V4 (w/o TransformerBackgroundBlock)
...
def parse_args():
    ...
    p.add_argument("--save-dir", type=str, default="output/RGS_Net_V4_noTrans/voting_202511252145", help="训练输出目录，用于推断权重路径和保存评估结果")
...
def main():
    ...
    model = RGSNetV4NoTransformer(n_channels=len(BANDS)).to(device)
    if is_main_process():
