...
示例：
CUDA_VISIBLE_DEVICES=0,1,2 torchrun --nproc_per_node=3 --master_port=65530 exp/eval_scripts/eval_UNetPlusPlus.py --dist --data-root data/splits_merged_pixels --algo large --threshold 0.5 --save-dir output/UNetPlusPlus/large_202512260938 --extra-output --fire-category few --cat-thresholds 10 100 1000 --samples-per-cat 3
CUDA_VISIBLE_DEVICES=5 python exp/eval_scripts/eval_UNetPlusPlus.py --data-root data/splits_merged_pixels --algo large --bands 7 6 5 --threshold 0.5 --save-dir output/UNetPlusPlus/large_202512231016
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate UNet++ (aligned with V4 report style)")
    ...
    p.add_argument("--save-dir", type=str, default="output/UNetPlusPlus/voting_date", help="训练输出目录，用于推断权重路径和保存评估结果")
...
def main():
    ...
    model = UNetPlusPlus(n_channels=len(BANDS), n_classes=1).to(device)
    if is_main_process():
