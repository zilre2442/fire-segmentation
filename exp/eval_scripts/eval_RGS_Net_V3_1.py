...
ALGORITHM = 'voting'
SAVE_DIR = "output/RGS_Net_V3_1/voting_few_202511161347"
BANDS = (7, 6, 5)
...
#
#    CUDA_VISIBLE_DEVICES=1 torchrun --nproc_per_node=1 exp/eval_scripts/eval_RGS_Net_V3_1.py --dist --batch-size 64 --save-dir output/RGS_Net_V3_1/voting_date
#
#    评估按火点数量划分的子数据集（例如 very_few 类别）:
#
#    torchrun --nproc_per_node=1 exp/eval_scripts/eval_RGS_Net_V3_1.py --dist --fire-category very_few --save-dir output/RGS_Net_V3_1/voting_very_few_date
#
#    评估 many 类别:
#
#    torchrun --nproc_per_node=1 exp/eval_scripts/eval_RGS_Net_V3_1.py --dist --fire-category many --save-dir output/RGS_Net_V3_1/voting_many_date
#
# 2) 单卡评估（手动设置环境变量）:
#
#    RANK=0 WORLD_SIZE=1 LOCAL_RANK=0 python3 exp/eval_scripts/eval_RGS_Net_V3_1.py --batch-size 64 --save-dir output/RGS_Net_V3_1/voting_date
#
#    评估 few 类别:
#
#    RANK=0 WORLD_SIZE=1 LOCAL_RANK=0 python3 exp/eval_scripts/eval_RGS_Net_V3_1.py --fire-category few --save-dir output/RGS_Net_V3_1/voting_few_date
#
# 3) 其他可选参数:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Distributed evaluation for RGSNetV3_1")
...
if is_main_process():
    ...
    with open(report_path, "w") as f:
        f.write("=" * 50 + "\n")
        f.write("RGS-Net V3_1 火点检测模型评估报告\n")
        f.write("=" * 50 + "\n\n")
