"""评估脚本: Dual-Sight Fire StageFusion 变体

- 支持分布式评估与聚合
- 默认从 `--save-dir/weights/model_best.pth` 载入；可用 `--model-path` 指定
- 可选择输出类型 fused / seg 进行阈值化评估
- 指标: Precision / Recall / F1 / IoU(Fire/Back) / mIoU
- 保存可视化样例 (BANS/GT/Seg/Rec/Fused) 及可选火点类别样例

示例:
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=65530 exp/eval_scripts/eval_DualSight_Fire_stagefusion.py --data-root data/splits_activefire --algo voting --save-dir output/DualSight_Fire_stagefusion/voting_202512191353
CUDA_VISIBLE_DEVICES=7 python exp/eval_scripts/eval_DualSight_Fire_stagefusion.py --data-root data/splits_activefire --algo voting --bands 7 6 5 --output-type fused --save-dir output/DualSight_Fire_stagefusion/voting_202512191353
"""
...
from models.DualSight_Fire import DualSight_FireStageFusion
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate Dual-Sight Fire StageFusion")
    ...
    p.add_argument("--save-dir", type=str, default="output/DualSight_Fire_stagefusion/voting_date", help="输出目录")
    ...
    p.add_argument("--samples-per-cat", type=int, default=5, help="每个类别样例数")
    p.add_argument("--samples", type=int, default=5, help="主可视化样例数量")
    p.add_argument("--extra-output", action="store_true", help="保存按火点数量类别的附加样例")
    return p.parse_args()
...
def main():
    if _is_preinit_main():
        print("========== Dual-Sight Fire StageFusion 火灾检测评估启动 ==========")
        print(f"当前时间: {time.strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"计算设备: {'GPU可用' if torch.cuda.is_available() else '仅限CPU'}")

    args = parse_args()
    test_csv = get_test_csv(args.data_root, args.algo, args.fire_category)
    if _is_preinit_main():
        print("\n[阶段 1/4] 准备测试数据")
        print(f"├─ 算法标签: {args.algo}")
    ...
    param_path = args.model_path or os.path.join(args.save_dir, "weights", f"{model_name}.pth")
    if is_main_process():
        print(f"├─ 参数路径: {param_path}")

    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = DualSight_FireStageFusion(n_channels=len(args.bands)).to(device)
    if is_main_process():
        print(f"└─ 网络架构: {model.__class__.__name__}")
        print(f"└─ 重建权重参数: {model.rec_weight}")
        mid_weights = ", ".join(f"{w.item():.3f}" for w in model.mid_fuse_weights)
        print(f"└─ 中间融合权重: [{mid_weights}]")
    ...
    if is_main_process():
        ...
        eval_mid = ", ".join(f"{w.item():.3f}" for w in model.mid_fuse_weights)
        print(f"└─ 中间融合权重: [{eval_mid}]")
    ...
    total_tp = total_fp = total_fn = total_px = num_samples = 0
    ...
    with torch.inference_mode():
        ...
        for images, masks in iterator:
            ...
            chosen = fused_logits if args.output_type == 'fused' else seg_logits
    ...
    report_path = os.path.join(args.save_dir, f"{result_filename}.txt")
    if is_main_process():
        with open(report_path, "w") as f:
            f.write("=" * 50 + "\n")
            f.write("Dual-Sight Fire StageFusion 火点检测模型评估报告\n")
            f.write("=" * 50 + "\n\n")
            f.write(f"设备: {device}\n")
            f.write(f"测试样本数: {num_samples_g}\n")
            f.write(f"重建权重参数: {eval_model.rec_weight.item()}\n")
            mid_vals = ", ".join(f"{w.item():.4f}" for w in eval_model.mid_fuse_weights)
            f.write(f"中间融合权重: [{mid_vals}]\n\n")
            f.write("=" * 50 + "\n")
            f.write("评估指标\n")
            f.write("=" * 50 + "\n")
    ...
    if is_main_process():
        print("\n" + "=" * 50)
        print("Dual-Sight Fire StageFusion 火点检测模型评估结果:")
        ...
        mid_vals = ", ".join(f"{w.item():.4f}" for w in eval_model.mid_fuse_weights)
        print(f" - 中间融合权重: [{mid_vals}]")
        print(f"\n评估结果已保存至: {args.save_dir}")
    ...
    if is_main_process():
        ...
        plt.tight_layout()
        vis_path = os.path.join(args.save_dir, f"{model_name}_prediction.png")
        ...
        if args.extra_output:
            ...
            CATEGORY_DIR = os.path.join(args.save_dir, "category_samples")
            ...
            def plot_category(cat_name: str, items: List[Tuple[np.ndarray, np.ndarray, np.ndarray, int, int]]):
                ...
                out_path = os.path.join(
                    CATEGORY_DIR,
                    f"cat_{cat_name}_thr_{thr1}-{thr2}-{thr3}_k{samples_per_cat}.png",
                )
