"""评估脚本: RGS-Net V4 StageFusion 变体
...
示例:
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=65530 exp/eval_scripts/eval_RGS_Net_V4_stagefusion.py --data-root data/splits_activefire --algo voting --save-dir output/RGS_Net_V4_stagefusion/voting_202512191353
CUDA_VISIBLE_DEVICES=7 python exp/eval_scripts/eval_RGS_Net_V4_stagefusion.py --data-root data/splits_activefire --algo voting --bands 7 6 5 --output-type fused --save-dir output/RGS_Net_V4_stagefusion/voting_202512101103
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate RGS-Net V4 StageFusion")
    ...
    p.add_argument("--save-dir", type=str, default="output/RGS_Net_V4_stagefusion/voting_date", help="输出目录")
...
def main():
    if _is_preinit_main():
        print("========== RGS-Net V4 StageFusion 火灾检测评估启动 ==========")
    ...
    model = RGSNetV4StageFusion(n_channels=len(args.bands)).to(device)
    if is_main_process():
    ...
    if is_main_process():
        with open(report_path, "w") as f:
            f.write("=" * 50 + "\n")
            f.write("RGS-Net V4 StageFusion 火点检测模型评估报告\n")
            f.write("=" * 50 + "\n\n")
            f.write(f"设备: {device}\n")
    ...
    if is_main_process():
        print("\n" + "=" * 50)
        print("RGS-Net V4 StageFusion 火点检测模型评估结果:")
