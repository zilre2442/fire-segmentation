"""评估脚本: Attention UNet (与 Dual-Sight Fire 输出对齐)

- 支持分布式评估 (`--dist`)，自动聚合全局指标
- 默认从 `--save-dir/weights/model_best.pth` 加载参数；也支持 `--model-path`
- 指标：Precision / Recall / F1 / IoU(Fire/Back) / mIoU
- 保存样例：pred_samples (BANS/GT/Pred) 与整体可视化；可选附加按火点数量类别输出（启用 `--extra-output`）

示例：
CUDA_VISIBLE_DEVICES=0,1,2,7 torchrun --nproc_per_node=4 --master_port=65531 exp/eval_scripts/eval_AttentionUNet.py --dist --data-root data/splits_merged_pixels --algo large --save-dir output/AttentionUNet/large_202512261505 --extra-output --fire-category few --cat-thresholds 10 100 1000 --samples-per-cat 3
CUDA_VISIBLE_DEVICES=5 python exp/eval_scripts/eval_AttentionUNet.py --data-root data/splits_merged_pixels --algo large --bands 7 6 2 --threshold 0.5 --save-dir output/AttentionUNet/large_202512231016
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate Attention UNet (aligned with V4 report style)")
    ...
    p.add_argument("--save-dir", type=str, default="output/AttentionUNet/voting_date", help="训练输出目录，用于推断权重路径和保存评估结果")
    ...
    p.add_argument(
        ...
        help="按火点数量选择子集，可传多个，如 --fire-category very_few few",
    )
...
def get_test_paths(root: str, algo: str, fire_categories: Optional[List[str]]) -> List[str]:
    if not fire_categories:
        return [get_test_csv(root, algo, None)]
    return [get_test_csv(root, algo, cat) for cat in fire_categories]
...
def main():
    ...
    test_paths = get_test_paths(args.data_root, ALGO, args.fire_category)
    if _is_preinit_main():
        print("\n[阶段 1/4] 准备测试数据")
        cat_info = ",".join(args.fire_category) if args.fire_category else "全量数据集"
        print(f"├─ 算法标签: {ALGO}")
        print(f"├─ 火点类别: {cat_info}")
        print(f"├─ 测试集CSV: {test_paths}")

    ds_list = [LandsatFireDataset(p, bands=BANDS) for p in test_paths]
    ...
    model = AttentionUNet(n_channels=len(BANDS), n_classes=1).to(device)
    if is_main_process():
    ...
    with torch.inference_mode():
        ...
        for images, masks in iterator:
            ...
            # 模型现返回 logits，评估时显式 sigmoid 得到概率
            probs = torch.sigmoid(eval_model(images))
            preds = (probs >= args.threshold).bool()
            gt = masks.bool()
    ...
    cat_suffix = None
    if args.fire_category:
        cat_suffix = "_" + "-".join(args.fire_category)
    result_filename = f"eval_{'model_best' if args.model_path is None else os.path.splitext(os.path.basename(args.model_path))[0]}{cat_suffix or ''}"
    report_path = os.path.join(SAVE_DIR, f"{result_filename}.txt")
    if is_main_process():
        with open(report_path, "w") as f:
            f.write("=" * 50 + "\n")
            f.write("Attention UNet 火点检测模型评估报告\n")
            f.write("=" * 50 + "\n\n")
            f.write(f"设备: {device}\n")
    ...
    if is_main_process():
        print("\n" + "=" * 50)
        print("Attention UNet 火点检测模型评估结果:")
    ...
    if is_main_process():
        ...
        with torch.inference_mode():
            probs = torch.sigmoid(eval_model(images))
            pred_masks = (probs >= args.threshold).float().cpu().numpy()
        ...
        for idx in range(imgs_np.shape[0]):
            ...
            pred_img = (pred_masks[idx][0] * 255).astype(np.uint8)
            __import__('PIL').Image.fromarray(pred_img).save(os.path.join(SAMPLE_DIR, f"sample_{idx:02d}_pred.png"))
        ...
        for i in range(rows):
            ...
            axes[i, 1].imshow(gts_np[i][0], cmap="gray"); axes[i, 1].set_title("GT", fontsize=10); axes[i, 1].axis("off")
            axes[i, 2].imshow(pred_masks[i][0], cmap="gray"); axes[i, 2].set_title("Pred", fontsize=10); axes[i, 2].axis("off")
        ...
        if args.extra_output:
            ...
            with torch.inference_mode():
                for images_b, masks_b in __import__('tqdm').tqdm(full_loader, desc="挑选样例"):
                    ...
                    preds_b = (torch.sigmoid(eval_model(images_b)) >= args.threshold).float()
