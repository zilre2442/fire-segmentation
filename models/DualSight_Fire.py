...
与现有方法对比:
- 相较于传统 UNet，Dual-Sight Fire 在背景建模上更具优势。
- 双分支设计提升了模型的鲁棒性。
...
# 主模型: DualSight_Fire
# -------------------------------------------------
class DualSight_Fire(nn.Module):
    """
    Dual-Sight Fire 主模型。
...
class DualSight_FireStageFusion(DualSight_Fire):
    """Dual-Sight Fire variant with per-stage fusion between segmentation and reconstruction decoders."""
...
__all__ = ["DualSight_Fire", "TransformerBackgroundBlock", "DualSight_FireStageFusion"]
...
if __name__ == "__main__":
    ...
    model = DualSight_Fire(n_channels=3, n_classes=1, base_filters=64, dropout=0.1, batchnorm=True)
