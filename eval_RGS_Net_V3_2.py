...
from models.RGS_Net_V3_2 import RGSNetV3_1
...
ALGORITHM = 'voting'
SAVE_DIR = "output/RGS_Net_V3_2/voting_202511242001"
BANDS = (7, 6, 5)
...
#
#    CUDA_VISIBLE_DEVICES=1,2 torchrun --nproc_per_node=2 eval_RGS_Net_V3_2.py --dist --batch-size 64 --save-dir output/RGS_Net_V3_2/voting_date
#
#    评估按火点数量划分的子数据集（例如 very_few 类别）:
#
#    torchrun --nproc_per_node=1 eval_RGS_Net_V3_2.py --dist --fire-category very_few --save-dir output/RGS_Net_V3_2/voting_very_few_date
#
#    评估 many 类别:
#
#    torchrun --nproc_per_node=1 eval_RGS_Net_V3_1.py --dist --fire-category many --save-dir output/RGS_Net_V3_2/voting_many_date
#
# 2) 单卡评估（手动设置环境变量）:
#
#    RANK=0 WORLD_SIZE=1 LOCAL_RANK=0 python3 eval_RGS_Net_V3_1.py --batch-size 64 --save-dir output/RGS_Net_V3_2/voting_date
#
#    评估 few 类别:
#
#    RANK=0 WORLD_SIZE=1 LOCAL_RANK=0 python3 eval_RGS_Net_V3_1.py --fire-category few --save-dir output/RGS_Net_V3_2/voting_few_date
#
# 3) 其他可选参数:
#    --algo: 数据集算法类型 (voting, Kumar-Roy, Murphy, Schroeder, intersection, Land8Fire)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Distributed evaluation for RGSNetV3_2")
...
def get_test_csv_path(data_root: str, algorithm: str, fire_category: str = None):
    """
    获取测试集CSV文件路径
    ...
    if fire_category is None:
        # 使用全量数据集
        test_csv = os.path.join(data_root, f"{algorithm}_test.csv")
    else:
        # 使用按火点数量划分的子数据集
        ...
        test_csv = os.path.join(subdir, "test", f"{algorithm}_test_{fire_category}.csv")
...
test_data_csv = get_test_csv_path(DATA_ROOT, ALGORITHM, FIRE_CATEGORY)
if _is_preinit_main():
    print(f"├─ 算法标签: {ALGORITHM}")
    print(f"├─ 火点类别: {FIRE_CATEGORY if FIRE_CATEGORY else '全量数据集'}")
...
model = RGSNetV3_1(n_channels=len(BANDS), n_filters=64)
if is_main_process():
...
if is_main_process():
    ...
    with open(report_path, "w") as f:
        f.write("=" * 50 + "\n")
        f.write("RGS-Net V3_2 火点检测模型评估报告\n")
        f.write("=" * 50 + "\n\n")
...
if is_main_process():
    ...
    with torch.inference_mode():
        ...
        # 提取多尺度输出
        multi_scale_seg = [torch.sigmoid(logits).cpu().numpy() for logits in outputs.multi_scale_seg_logits]
        multi_scale_rec = [torch.sigmoid(logits).cpu().numpy() for logits in outputs.multi_scale_rec_logits]
    ...
    for idx in range(imgs_np.shape[0]):
        ...
        # 保存多尺度分割输出
        for scale_idx, scale_output in enumerate(multi_scale_seg):
            scale_img = (scale_output[idx][0] * 255).astype(np.uint8)
            Image.fromarray(scale_img).save(os.path.join(SAMPLE_DIR, f"sample_{idx:02d}_seg_scale{scale_idx}.png"))
        
        # 保存多尺度重建输出
        for scale_idx, scale_output in enumerate(multi_scale_rec):
            scale_img = (scale_output[idx][0] * 255).astype(np.uint8)
            Image.fromarray(scale_img).save(os.path.join(SAMPLE_DIR, f"sample_{idx:02d}_rec_scale{scale_idx}.png"))
    ...
    # 多尺度对比可视化（选择第一个样本）
    if len(multi_scale_seg) > 0:
        num_scales = len(multi_scale_seg)
        # 每行显示：GT, 分割各尺度, 重建各尺度, Fused
        fig_ms, axes_ms = plt.subplots(3, num_scales + 2, figsize=(3 * (num_scales + 2), 9))
        
        sample_idx = 0  # 选择第一个样本
        
        # 第一行：RGB和GT
        rgb_display = imgs_np[sample_idx][:3]
        rgb_display = (rgb_display * 255).clip(0, 255).astype(np.uint8)
        rgb_display = np.transpose(rgb_display, (1, 2, 0))
        axes_ms[0, 0].imshow(rgb_display)
        axes_ms[0, 0].set_title("Input RGB", fontsize=10)
        axes_ms[0, 0].axis("off")
        
        axes_ms[0, 1].imshow(gts_np[sample_idx][0], cmap="gray")
        axes_ms[0, 1].set_title("Ground Truth", fontsize=10)
        axes_ms[0, 1].axis("off")
        
        # 显示分割多尺度
        for scale_idx in range(num_scales):
            axes_ms[0, scale_idx + 2].imshow(multi_scale_seg[scale_idx][sample_idx][0], cmap="gray")
            axes_ms[0, scale_idx + 2].set_title(f"Seg Scale {scale_idx}", fontsize=10)
            axes_ms[0, scale_idx + 2].axis("off")
        
        # 第二行：重建多尺度
        axes_ms[1, 0].axis("off")
        axes_ms[1, 1].axis("off")
        for scale_idx in range(num_scales):
            axes_ms[1, scale_idx + 2].imshow(multi_scale_rec[scale_idx][sample_idx][0], cmap="gray")
            axes_ms[1, scale_idx + 2].set_title(f"Rec Scale {scale_idx}", fontsize=10)
            axes_ms[1, scale_idx + 2].axis("off")
        
        # 第三行：最终输出对比
        axes_ms[2, 0].imshow(seg_mask[sample_idx][0], cmap="gray")
        axes_ms[2, 0].set_title("Final Seg", fontsize=10)
        axes_ms[2, 0].axis("off")
        
        axes_ms[2, 1].imshow(rec_mask[sample_idx][0], cmap="gray")
        axes_ms[2, 1].set_title("Final Rec", fontsize=10)
        axes_ms[2, 1].axis("off")
        
        axes_ms[2, 2].imshow(fused_mask[sample_idx][0], cmap="gray")
        axes_ms[2, 2].set_title("Fused Output", fontsize=10)
        axes_ms[2, 2].axis("off")
        
        # 隐藏剩余的子图
        for col_idx in range(3, num_scales + 2):
            axes_ms[2, col_idx].axis("off")
        
        plt.tight_layout()
        ms_vis_path = os.path.join(SAVE_DIR, f"{model_name}_multiscale.png")
        plt.savefig(ms_vis_path, dpi=300, bbox_inches="tight")
        plt.close(fig_ms)
        print(f"多尺度可视化已保存: {ms_vis_path}")
